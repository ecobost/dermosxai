""" Takes the waterfall plot from SHAP and modifies slightly

Changes:
    Color the features according 
"""
...
def waterfall(shap_values, max_display=10, show=True):
    ...
    if (type(base_values) == np.ndarray and
            len(base_values) > 0) or type(base_values) == list:
        raise Exception(
    ...
    for i in range(num_individual):
        ...
        if num_individual != num_features or i + 4 < num_individual:
            plt.plot([loc, loc], [rng[i] - 1 - 0.4, rng[i] + 0.4], color="#bbbbbb",
                     linestyle="--", linewidth=0.5, zorder=-1)
        if features is None:
            yticklabels[rng[i]] = feature_names[order[i]]
        else:
            if np.issubdtype(type(features[order[i]]), np.number):
                yticklabels[rng[i]] = format_value(float(
                    features[order[i]]), "%0.03f") + " = " + feature_names[order[i]]
            else:
    ...
    for i in range(len(pos_inds)):
        dist = pos_widths[i]
        arrow_obj = plt.arrow(pos_lefts[i], pos_inds[i], max(dist - hl_scaled, 0.000001),
                              0, head_length=min(dist, hl_scaled), color=colors.red_rgb,
                              width=bar_width, head_width=bar_width)

        if pos_low is not None and i < len(pos_low):
            plt.errorbar(
                pos_lefts[i] + pos_widths[i],
                pos_inds[i], xerr=np.array([[pos_widths[i] - pos_low[i]],
                                            [pos_high[i] - pos_widths[i]]]),
                ecolor=colors.light_red_rgb)

        txt_obj = plt.text(pos_lefts[i] + 0.5 * dist, pos_inds[i],
                           format_value(pos_widths[i],
                                        '%+0.02f'), horizontalalignment='center',
        ...
        if text_bbox.width > arrow_bbox.width:
            ...
            txt_obj = plt.text(pos_lefts[i] + (5 / 72) * bbox_to_xscale + dist,
                               pos_inds[i], format_value(pos_widths[i], '%+0.02f'),
                               horizontalalignment='left', verticalalignment='center',
                               color=colors.red_rgb, fontsize=12)

    # draw the negative arrows
    for i in range(len(neg_inds)):
        dist = neg_widths[i]

        arrow_obj = plt.arrow(neg_lefts[i], neg_inds[i],
                              -max(-dist - hl_scaled, 0.000001), 0,
                              head_length=min(-dist, hl_scaled), color=colors.blue_rgb,
                              width=bar_width, head_width=bar_width)

        if neg_low is not None and i < len(neg_low):
            plt.errorbar(
                neg_lefts[i] + neg_widths[i],
                neg_inds[i], xerr=np.array([[neg_widths[i] - neg_low[i]],
                                            [neg_high[i] - neg_widths[i]]]),
                ecolor=colors.light_blue_rgb)

        txt_obj = plt.text(neg_lefts[i] + 0.5 * dist, neg_inds[i],
                           format_value(neg_widths[i],
                                        '%+0.02f'), horizontalalignment='center',
                           verticalalignment='center', color="white", fontsize=12)
        text_bbox = txt_obj.get_window_extent(renderer=renderer)
        arrow_bbox = arrow_obj.get_window_extent(renderer=renderer)
        ...
        if text_bbox.width > arrow_bbox.width:
            ...
            txt_obj = plt.text(neg_lefts[i] - (5 / 72) * bbox_to_xscale + dist,
                               neg_inds[i], format_value(neg_widths[i], '%+0.02f'),
                               horizontalalignment='right', verticalalignment='center',
                               color=colors.blue_rgb, fontsize=12)
    ...
    plt.axvline(base_values, 0, 1 / num_features, color="#bbbbbb", linestyle="--",
    ...
    ##################################### MY CHANGES ##################################
    # Plot attributs in color  depending on their value
    is_attribute = lambda x: ':' in x # ad-hoc
    for i in range(1, num_features): # start at 1 to avoid the xxx other features label
        feature_value, feature_name = tick_labels[i].get_text().split('=')
        if is_attribute(feature_name):
            if float(feature_value) < 0.25:
                feature_color = '#FF0000'
            elif float(feature_value) < 0.5:
                feature_color = '#AA5500'  
            elif float(feature_value) < 0.75:
                feature_color = '#55AA00'
            else:
                feature_color = '#00FF00'
            tick_labels[i + num_features].set_color(feature_color)
